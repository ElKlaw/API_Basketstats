...
public interface MatchRepository extends PagingAndSortingRepository<Match, Integer> {
	//Match Equipe
	@Query(value="Select e From Event e join e.match m join m.equipe eq where eq.id=?1 order by m.dateMatch desc, m.heureMatch desc")
	Page<Event> getAllMatchFromEquipe(int id, Pageable pageable);
	
	@Query(value="Select e From Event e join e.match m join m.equipe eq where eq.id=?1 and (m.dateMatch>CURRENT_DATE or (m.dateMatch=CURRENT_DATE and m.heureMatch>CURRENT_DATE)) order by m.dateMatch asc, m.heureMatch asc")
	Page<Event> getMatchFromEquipeFutur(int id, Pageable pageable);
	
	@Query(value="Select e From Event e join e.match m join m.equipe eq where eq.id=?1 and (m.dateMatch<CURRENT_DATE or (m.dateMatch=CURRENT_DATE and m.heureMatch<CURRENT_DATE)) order by m.dateMatch desc, m.heureMatch desc")
	Page<Event> getMatchFromEquipePasse(int id, Pageable pageable);
